...
namespace Warner
	{
	public class WorldTilingVertical : MonoBehaviour
		{
		...
		public static WorldTilingVertical instance;
		...
		private IEnumerator<float> createBuddiesCoRoutine()
			...
			for (int i = 0; i<layers.Length; i++)
				...
				layerData.spriteWidth = layerData.mainBuddySpriteRenderer.sprite.bounds.size.y
				...
				Vector3 leftPositionOffset = new Vector3(0, 
				...
				Vector3 centerPositionOffset = new Vector3(layerData.leftBuddy.transform.position.x, layerData.leftBuddy.transform.position.y + layerData.spriteWidth, 
				...
				Vector3 rightPositionOffset = new Vector3(layerData.centerBuddy.transform.position.x, layerData.centerBuddy.transform.position.y + layerData.spriteWidth, layerData.centerBuddy.transform.position.z);
		...
		private void generateColliders(LayerData layerData)
			...
			for (int i=0; i<colliderPointsList.Count; i++)
				...
				newPoint.y += layerData.spriteWidth;
		...
		private void onCameraMove()
			...
			for (int i = 0; i<layers.Length; i++)
				...
				//swap stuff
				float camHorizontalExtend = CameraController.instance.cam.orthographicSize*Screen.height/Screen.width;
				camHorizontalExtend = CameraController.instance.worldSize.y*0.5f;
				
				float edgeVisiblePositionRight = (layers[i].centerBuddy.transform.position.y+
					layers[i].spriteWidth+layers[i].spriteWidth/2)-camHorizontalExtend;
				float edgeVisiblePositionLeft = (layers[i].centerBuddy.transform.position.y
					-layers[i].spriteWidth-layers[i].spriteWidth/2)+camHorizontalExtend;

				if (CameraController.instance.cam.transform.position.y+OffsetX>=edgeVisiblePositionRight)
					{
					Vector3 newPosition = new Vector3(layers[i].mainBuddySpriteRenderer.transform.position.x, layers[i].rightBuddy.transform.position.y +
						(layers[i].spriteWidth), 
					...
					{
					if (CameraController.instance.cam.transform.position.y-OffsetX<=edgeVisiblePositionLeft)
						{
						Vector3 newPosition = new Vector3(layers[i].mainBuddySpriteRenderer.transform.position.x, layers[i].leftBuddy.transform.position.y - layers[i].spriteWidth,
							layers[i].mainBuddySpriteRenderer.transform.position.z);
